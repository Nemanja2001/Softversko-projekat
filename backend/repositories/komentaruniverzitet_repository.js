...
const getCommentByUniversity = async (UniversityID)=>{
    const resluts = await pool.query('Select "Name","Surname", "Tekst", "Ocjena","IDKorisnika" from public."KomentarUniverzitet" inner join public."Korisnici" on "ID"="IDKorisnika" where "IDUniverziteta" = $1',[UniversityID]);
    return resluts.rows
}
