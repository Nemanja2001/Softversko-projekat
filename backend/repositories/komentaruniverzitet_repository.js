...
const getCommentByUser = async (UserID)=>{
    const resluts = await pool.query('Select Ime from public."KomentarUniverzitet" where IDKorisnika = $1',[UserID]);
    return resluts.rows
}

const getCommentByUniversity = async (UniversityID)=>{
    const resluts = await pool.query('Select Ime from public."KomentarUniverzitet" where IDUniverziteta = $1',[UniversityID]);
    return resluts.rows
}
...
const getNumOfComments = async (UniversityID)=>{
    const resluts = await pool.query('Select count(*) as ukupnoKomentara from public."KomentarUniverzitet" where IDUniverziteta = $1',[UniversityID]);
    return resluts.rows
}
